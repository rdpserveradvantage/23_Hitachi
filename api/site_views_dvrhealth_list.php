...
if($atmid!=''){
$sql = mysqli_query($con,"select * from network_report_list where ATMID='".$atmid."' and dvr_status='".$login_status."'"); 
}else{
	if($bank!=''){
		if($circle!=''){
	...
	$offlinetestsql = "SELECT * FROM network_report_list WHERE ATMID IN (".$atmidarray.") and dvr_status='".$login_status."'";
                        ...
                        $count = 0 ; 
						  if(mysqli_num_rows($sql)>0){
							  while($sql_result = mysqli_fetch_assoc($sql)){
								...
								$_data['ip'] = $sql_result['dvr_ip'];
								$_data['last_communication'] = $sql_result['dvr_lastcommunication'];
