...
include('db_connection_38server.php'); 
$con = OpenCon();
$ems_login_sql = mysqli_query($con,"select email,password,access_token,org_id,refresh_token from ems_login_access_panel_health where id=2");
...
function user_login($email,$password,$con){
	$curl = curl_init();
	...
	if(count($headers)>0){
		...
		//	$sql= " UPDATE `eazyinfra_login_access` SET `access_token`='".$access_token."',`refresh_token`='".$refresh_token."',`updated_at`='".$currentdatetime."' where `id`='".$id."' ";
            $sql= " UPDATE `ems_login_access_panel_health` SET `access_token`='".$access_token."',`refresh_token`='".$refresh_token."',`updated_at`='".$currentdatetime."' where `id`='".$id."' ";        
...
function getZone($org_id,$panel_id,$access_token,$con){
   ...
   // echo '<pre>';print_r($json);echo '</pre>';die;
	curl_setopt_array($curl, array(
	  CURLOPT_URL => 'http://103.141.218.138:4511/panel/zonedata',
	  CURLOPT_RETURNTRANSFER => true,
	  CURLOPT_ENCODING => '',
	  CURLOPT_MAXREDIRS => 10,
	  CURLOPT_TIMEOUT => 0,
...
function getHooterSirenStatus($org_id,$panel_id,$group_id,$access_token,$con){
   ...
   // echo '<pre>';print_r($json);echo '</pre>';
	curl_setopt_array($curl, array(
	  CURLOPT_URL => 'http://103.141.218.138:4511/panel/configs/get_new',
	  CURLOPT_RETURNTRANSFER => true,
	  CURLOPT_ENCODING => '',
	  CURLOPT_MAXREDIRS => 10,
	  CURLOPT_TIMEOUT => 0,
...
function setHooterSiren($org_id,$mac_id,$onoff_type,$set_type,$access_token,$con){
   ...
   // echo '<pre>';print_r($json);echo '</pre>';
	curl_setopt_array($curl, array(
	  CURLOPT_URL => 'http://103.141.218.138:4511/diagnostics/cmd_new',
	  CURLOPT_RETURNTRANSFER => true,
	  CURLOPT_ENCODING => '',
	  CURLOPT_MAXREDIRS => 10,
	  CURLOPT_TIMEOUT => 0,
...
function getPanelRelayData($org_id,$mac_id,$access_token,$con){
	$curl = curl_init();
	//$panel_id_arr = array();
	//array_push($panel_id_arr,$panel_id);
	$mac_id_array = array();
	array_push($mac_id_array ,$mac_id);
	$foo = new StdClass();
	$foo->mac_id = $mac_id_array;
	$foo->org_id = (int)$org_id;
	$foo->config_type = 'schedule';
...
function getPanelZoneData($org_id,$group_id,$mac_id,$access_token,$con){
	$curl = curl_init();
	//$panel_id_arr = array();
	//array_push($panel_id_arr,$panel_id);
	$mac_id_array = array();
