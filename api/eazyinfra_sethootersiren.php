...
if(isset($_POST)){
	$onoff_type = 0;
	if(isset($_POST['onofftype'])){
        $onoff_type = $_POST['onofftype'];
	}
	$_ipaddress = "";
	if(isset($_POST['ipaddress'])){
        $_ipaddress = $_POST['ipaddress'];
	}
	$set_type = "";
	if(isset($_POST['settype'])){
        $set_type = $_POST['settype'];
	}
	...
	if($_ipaddress!='' && $set_type!=''){	
		...
		function getAccessToken($refresh_token){
			...
			function getLogin(){
				$curl = curl_init();
				//$postData = [  CURLOPT_URL => 'http://103.141.218.138:4511/user/login',
				//	'email' => 'api@cts.in',
				//	'password' => 'api@123'
				//];
				$postData = [
					'email' => 'api@utopiatech.in',
					'password' => 'Api@123'
				];
				curl_setopt_array($curl, array(
