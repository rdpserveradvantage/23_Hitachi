...
function get_sensor_name($zone,$panelid,$con,$alarm)
{
	...
	$banks = explode(",",$_SESSION['bankname']);
	 ...
	 $bank = "";
	   ...
	   $filters = "Bulk Ticket View Report Filter Data Client : ".$client;
...
if($portal=="all"){
	...
	b.createtime,b.receivedtime,b.comment,b.zone,b.alarm,b.closedBy,b.closedtime,b.sendip FROM (".$sitesql.") AS a,`alerts` b WHERE a.NewPanelID=b.panelid AND b.sendtoclient='S' AND CAST(b.createtime AS DATE)>='".$start_date."' AND CAST(b.createtime AS DATE)<='".$end_date."'";					  
}else{
	if($portal=="active"){
	   $sql ="SELECT  a.Customer,a.Bank,a.ATMID,a.ATMShortName,a.SiteAddress,a.DVRIP,a.Panel_make,a.zone as zon,a.City,a.State,b.id,b.panelid,
	b.createtime,b.receivedtime,b.comment,b.zone,b.alarm,b.closedBy,b.closedtime,b.sendip FROM (".$sitesql.") AS a,`alerts` b WHERE a.NewPanelID=b.panelid AND b.sendtoclient='S' AND CAST(b.createtime AS DATE)>='".$start_date."' AND CAST(b.createtime AS DATE)<='".$end_date."' AND b.status='O'";						  
	}else{
	   $sql ="SELECT  a.Customer,a.Bank,a.ATMID,a.ATMShortName,a.SiteAddress,a.DVRIP,a.Panel_make,a.zone as zon,a.City,a.State,b.id,b.panelid,
	b.createtime,b.receivedtime,b.comment,b.zone,b.alarm,b.closedBy,b.closedtime,b.sendip FROM (".$sitesql.") AS a,`alerts` b WHERE a.NewPanelID=b.panelid AND b.sendtoclient='S' AND CAST(b.createtime AS DATE)>='".$start_date."' AND CAST(b.createtime AS DATE)<='".$end_date."' AND b.status='C'";					  
	}
}
